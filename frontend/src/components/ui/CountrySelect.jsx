...
function CountrySelect({
  ...
  const selectedLabel = getCountryName(normalizedValue, language) || value || ''
  const resolvedPlaceholder = placeholder || t('common.country_select_placeholder')

  const options = useMemo(() => getCountryOptions(language), [language])
  const filteredOptions = useMemo(() => {
    const search = foldCountryText(query)
    if (!search) return options

    return options.filter((option) => (
      foldCountryText(option.label).includes(search)
      || foldCountryText(option.value).includes(search)
    ))
  }, [options, query])
              ...
              {filteredOptions.length > 0 ? (
                filteredOptions.map((option) => (
                  ...
                  >
                    ...
                    <span className="shrink-0 text-xs text-slate-400">{option.value}</span>
                  </button>
                ))
              ) : (
