...
// Prefer the server-provided widgetUrl (from /api/payment/epay-token); fall back to
// the env-based URL so dev mode still works without a server round-trip.
const loadHalykScript = (serverWidgetUrl) =>
    new Promise((resolve, reject) => {
        ...
        const isTest = import.meta.env.PROD
            ? import.meta.env.VITE_EPAY_TEST !== "false" && import.meta.env.VITE_PAYMENTS_LIVE === "false"
            : import.meta.env.VITE_EPAY_TEST !== "false";
        const fallbackSrc = isTest
            ? "https://test-epay.epayment.kz/payform/payment-api.js"
            : "https://epay.homebank.kz/payform/payment-api.js";
        const script = document.createElement("script");
        script.src = serverWidgetUrl || fallbackSrc;
...
function BookingModal({ isOpen, onClose, doctor }) {
    ...
    const handleEPayPayment = async () => {
        ...
        try {
            ...
            const { auth, terminalId, widgetUrl } = await tokenRes.json();

            // Load ePay widget script — use server-provided URL as source of truth
            await loadHalykScript(widgetUrl);
            ...
            {step < 4 ? (
                ...
                <Button
                    ...
                    {FREE_CONSULTATIONS
                        ...
                        ? t('common.book_appt')
                        : paymentMethod === "kaspi"
                        ? t('booking.book_kaspi', { price: displayPrice })
                        : paymentMethod === "halyk"
                        ? isMobileDevice()
                            ? t('booking.book_halyk_mobile', { price: displayPrice })
                            : t('booking.book_halyk_qr', { price: displayPrice })
                        : t('booking.book_pay', { price: displayPrice })}
                </Button>
            )}
        </div>
    ) : undefined
                            ...
                            <div className="bg-teal-50 rounded-xl px-4 py-3 text-sm text-teal-800 max-w-xs w-full">
                                {SHOW_DOCTOR_PRICES && (
                                    <p className="font-semibold mb-1">
                                        {t('booking.halyk_amount', { price: displayPrice })}
                                    </p>
                                )}
                        ...
                        <div className='bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 text-left'>
                            ...
                            {SHOW_DOCTOR_PRICES && (
                                <p className='text-sm text-amber-800 mb-1'>
                                    {t('booking.kaspi_amount', { price: displayPrice })}
                                </p>
                            )}
                        ...
                        {SHOW_DOCTOR_PRICES && (
                            <div className='sm:ml-auto sm:text-right w-full sm:w-auto'>
                                <p className='font-bold text-slate-900'>
                                    {FREE_CONSULTATIONS ? t('booking.free') : displayPrice}
                                </p>
                                        ...
                                        {medicalCases.map((item) => {
                                            ...
                                            return (
                                                <option key={id} value={id}>
                                                    {item.caseNumber || item.title || `Case #${item.id}`}
                                                </option>
                                            );
                                        })}
                                    </select>
                                    <p className='text-xs text-slate-500 mt-2'>
                                        Consultation will be attached to this treatment case.
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    {step === 4 && (
                        <div className='space-y-4'>
                            ...
                            <div className='bg-slate-50 rounded-xl divide-y divide-slate-200'>
                                ...
                                {selectedCaseId && (
                                    <div className='p-4 flex justify-between'>
                                        <span className='text-slate-600'>Medical case</span>
                                        <span className='font-medium text-slate-900'>
                                            {medicalCases.find((item) => (item.documentId || item.id) === selectedCaseId)?.caseNumber || 'Selected case'}
                                        </span>
                                    </div>
                                )}
                                {!FREE_CONSULTATIONS && (
