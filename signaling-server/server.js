...
const FREE_CONSULTATIONS = process.env.FREE_CONSULTATIONS !== 'false'
...
if (process.env.NODE_ENV === 'production' && !FREE_CONSULTATIONS && !PAYMENTS_LIVE && !ALLOW_TEST_PAYMENTS_IN_PRODUCTION) {
  console.error('[STARTUP] PAYMENTS_LIVE=true is required in production unless FREE_CONSULTATIONS=true or ALLOW_TEST_PAYMENTS_IN_PRODUCTION=true')
  process.exit(1)
}
if (process.env.NODE_ENV === 'production' && !FREE_CONSULTATIONS && !PAYMENTS_LIVE && ALLOW_TEST_PAYMENTS_IN_PRODUCTION) {
  console.warn('[STARTUP] Test payments are enabled in production; Halyk live payments are disabled')
}
if (!FREE_CONSULTATIONS && PAYMENTS_LIVE && missingPayment.length > 0) {
  ...
  try {
    ...
    const qrRes = await fetch(EPAY_QR_GENERATE_URL, {
      ...
      body: JSON.stringify({
        ...
        // In dev ePay can't reach localhost; SIGNALING_PUBLIC_URL should be set to the
        // prod server URL as placeholder. Payment confirmation relies on polling regardless.
        postLink: `${SIGNALING_PUBLIC_URL}/api/payment/epay-callback`,
        failurePostLink: `${SIGNALING_PUBLIC_URL}/api/payment/epay-failure-callback`,
        ...
        const apptRes = await fetch(`${STRAPI_API_URL}/api/appointments`, {
          ...
          body: JSON.stringify({
            data: {
              ...
              medical_case: pending.medical_case || null,
            },
          }),
        })
        if (!apptRes.ok) {
    ...
    const apptRes = await fetch(`${STRAPI_API_URL}/api/appointments`, {
      ...
      body: JSON.stringify({
        data: {
          ...
          medical_case: booking.medical_case || null,
...
app.post('/api/payment/epay-callback', (req, res) => {
  console.log(JSON.stringify({ audit: 'EPAY_CALLBACK', body: req.body, ts: new Date().toISOString() }))
  res.sendStatus(200)
})
